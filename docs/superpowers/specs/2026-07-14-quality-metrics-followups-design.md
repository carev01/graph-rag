...
2. **New edge `AvailableIn` (Product / Capability / Workload → Region)** (`ontology.py`): *a product, capability, or workload is available in, operates in, or stores data in a region.* Add to `EDGE_TYPES` and `EDGE_TYPE_MAP` as three pairs — `("Product","Region"): ["AvailableIn"]`, `("Capability","Region"): ["AvailableIn"]`, `("Workload","Region"): ["AvailableIn"]` — so residency can be expressed at product grain ("Azure Backup is available in Germany"), feature grain ("immutability is only offered in certain regions"), and workload grain ("SQL Server backup is supported in Germany West Central"). Add an `EXTRACTION_INSTRUCTIONS` line to capture availability/residency statements ("available in…", "data resides in…", "supported regions…", "not available in…"). This is what makes *"which vendors offer their SaaS backups in Germany?"* answerable: `(:Product)-[AvailableIn]->(:Region "Germany …")` (vendor reached via the structural layer / the product's `Provides` capability facts), and also supports the finer "is feature X available in Germany?" via the Capability/Workload pairs.
...
- `Region` availability is modelled via the `AvailableIn` edge from Product, Capability, and Workload. No region hierarchy (region→country→continent) or geo-reasoning; country rollup is retrieval-time containment. No `Vendor→Region` edge (vendor reached via the product).
