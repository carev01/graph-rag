...
- **Status note (2026-09-12):** ingest-time contradiction detection is currently *suspended by default* — the unfiltered invalidation-candidate search it needs is O(corpus) and was the ingestion scale wall, and the invalidations it produced were measured to be crawl-order artefacts (no document revision date exists in the corpus). Only same-pair contradictions can still invalidate at ingest; the sweep below is in practice the primary invalidation mechanism until a real time axis exists. See `docs/superpowers/specs/2026-09-12-suspend-contradiction-detection-design.md` (§4, §7) and BACKLOG 33. The policy in this section is unchanged as the target design.
